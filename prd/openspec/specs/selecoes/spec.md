...
## Purpose
Expor as 48 seleções participantes da Copa do Mundo 2026 e seus elencos: a página Equipes (`/equipes`) lista as seleções com busca por nome e filtro por grupo, e um modal apresenta técnico, posição no Ranking FIFA e o elenco convocado. Os dados vêm exclusivamente dos arquivos oficiais de `/fontes` (vinculados por código FIFA) e são consumidos pela UI por meio de serviço com DTOs, sem acesso direto ao EF Core nos componentes.
